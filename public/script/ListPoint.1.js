...
class ListPoint {
    ...
    displayList(elementID = this.idListElement) {
        ...
        const template = document.querySelector(`#${elementID}>template`);


        //clean the div without the template
        if (document.querySelectorAll(".point").length != 0) {
            for (let element of div.querySelectorAll(".point")) {
                element.remove();
            }
        }
        //function for make html element point from template
        function addPointElementInDiv(id, label) {
            const pointElement = template.content.cloneNode(true);

            pointElement.querySelector('.point').setAttribute("data-id", id);
            pointElement.querySelector('span').textContent = label;

            div.appendChild(pointElement);
        }

        //add to div the points elements from list
        for (let point of this.list) {

            addPointElementInDiv(point.properties.id, point.properties.label);
